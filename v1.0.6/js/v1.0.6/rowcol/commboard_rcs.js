...
class Keyboard {
    constructor(user_id, first_load, phase, prev_data) {
        ...
        this.input_method = prev_data.input_method;
        ...
        } else if (this.input_method === "left_mouse") {
    ...
    init_ui() {
        ...
        this.scan_delay_inc.onclick = function (e) {
        ...
        this.scan_delay_dec.onclick = function (e) {
            if (e) {     // ignore if PointerEvent or other Mouse related event (not triggered by "switch")
                return
            }
        ...
        this.extra_delay_inc.onclick = function (e) {
            if (e) {     // ignore if PointerEvent or other Mouse related event (not triggered by "switch")
                return
            }
        ...
        this.extra_delay_dec.onclick = function (e) {
            if (e) {     // ignore if PointerEvent or other Mouse related event (not triggered by "switch")
                return
            }
        ...
        if (this.user_id) {
            ...
            document.getElementById("info_label").innerHTML = `<b>Welcome user ${this.user_id}! Select "Options" and then "Start Session" to begin.</b>`;
        ...
        if (this.in_session) {
            options_array = [
                [new rowcolButton(this.scan_delay_dec, "1"), new rowcolButton(this.scan_delay_inc, "2"),
                    new rowcolButton(this.abort_options_button, "3")],
                [new rowcolButton(this.extra_delay_dec, "4"), new rowcolButton(this.extra_delay_inc, "5")]
            ];
        } else {
            options_array = [
                [new rowcolButton(this.scan_delay_dec, "1"), new rowcolButton(this.scan_delay_inc, "2"),
                    new rowcolButton(this.change_user_button, "3"), new rowcolButton(this.abort_options_button, "4")],
                [new rowcolButton(this.extra_delay_dec, "5"), new rowcolButton(this.extra_delay_inc, "6"),
                    new rowcolButton(this.tutorial_button, "7"), new rowcolButton(this.session_button, "8")]
            ];
        }
    ...
    on_press() {
        ...
        if (document.hasFocus()) {
            this.play_audio();
            if (this.in_tutorial) {
                this.tutorial_manager.on_press();
            } else if ((this.fetched_words || this.emoji_keyboard) && !this.in_info_screen && !this.in_webcam_info_screen && !this.in_finished_screen) {
                if (this.in_session) {
...
function send_login() {
    $.ajax({
        ...
        if (result.length > 0) {
            ...
            prev_data["input_method"] = input_method;
