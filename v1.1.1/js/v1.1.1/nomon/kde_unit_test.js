...
class Keyboard {
    constructor(user_id, first_load, phase, prev_data) {
        ...
        this.sim_press_resids = [];
        this.click_dist_divergences = [];
    }
    ...
    init_ui() {
        ...
        this.abort_options_button.onclick = function (e) {
            if (e) {     // ignore if PointerEvent or other Mouse related event (not triggered by "switch")
                return
            }
            this.destroy_options_rcom();
    ...
    make_choice(index) {
        ...
        console.log( kl_div(this.click_dist_true.slice(), this.bc.clock_inf.kde.dens_li.slice()))

        console.log(this.bc.clock_inf.kde.dens_li.slice())
        console.log(this.click_dist_true.slice())
    ...
    setup_simulation(){
        ...
        this.click_time_gen = function click_fun(){
            var bool = Math.random() < 0.5;
            return (randn_bm()*this.time_rotate/32 -0.5 +0.2)% (this.time_rotate / 2)*bool + (randn_bm()*this.time_rotate/32 +0.5 +0.2)% (this.time_rotate / 2)*(1-bool)
        }.bind(this);

        this.click_dist_true = []
        for (var i=0; i<80; i+=1){
            this.click_dist_true.push(normal(this.time_rotate*(i-40)/80/2, 2.5, (this.time_rotate/32/2)**2)+
            normal(this.time_rotate*(i-40)/80/2, -2.5, (this.time_rotate/32/2)**2))
        }

        this.press_rel_time = this.click_time_gen();

        this.bc.clock_inf.kde = new KernelDensityEstimation(this.time_rotate);
        this.bc.clock_inf.kde.initialize_zero_dens();
        for (var i=0; i < 20; i+=1) {
            var yin = this.click_time_gen();
    ...
    update_sim(){
        var target_index = this.bc.clock_inf.clock_util.cur_hours[0];
        var target_rel_time = this.time_rotate*(target_index-40)/80;

        if(Math.abs(target_rel_time - this.press_rel_time) < 0.025){
            this.sim_press_resids.push(target_rel_time - this.press_rel_time);
            this.simulate_press()
            // console.log(arrAvg(this.sim_press_resids))
        }
...
function renormalize(h) {
    var normalizer = h.reduce((a,b) => a + b, 0);
    for (var i in h) {
        h[i] = h[i] / normalizer;
    }
}
...
function kl_div(h1, h2){
    renormalize(h1);
    renormalize(h2);
    var sum = 0;
    for (var i=0; i < h1.length; i+=1){
        sum += h1[i]*Math.log(h1[i]/h2[i]);
    }
    return sum;
}

let keyboard = new Keyboard(user_id, first_load, phase, null);
